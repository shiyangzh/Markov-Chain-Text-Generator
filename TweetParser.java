...
public class TweetParser {
    ...
    static String extractColumn(String csvLine, int csvColumn) {
        ...
        if (csvColumn >= arr.length) {
            return null;
        }
        String line = arr[csvColumn];
        return line;
    }
    ...
    static List<String> csvFileToTweets(String pathToCSVFile, int tweetColumn) {
        ...
        while (iterator.hasNext()) {
            ...
            String a = extractColumn(s, tweetColumn);
            if (a != null) {
                tweets.add(a);
            }
        }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        List<String> cleanedTweet = tweetSplit(removeURLs(tweet));
        List<List<String>> cleanedSentences = new LinkedList<List<String>>();
        for (String s : cleanedTweet) {
            List<String> cleaned = parseAndCleanSentence(s);
            if (!cleaned.isEmpty()) {
    ...
    public static List<List<String>> csvFileToTrainingData(String pathToCSVFile, int tweetColumn) {
        ...
        for (String s : tweets) {
            List<List<String>> sentence = (parseAndCleanTweet(s));
            trainingData.addAll(sentence);
        }
        return trainingData;
    }
