...
public class TwitterBot {
    ...
    public String generateTweet(int numWords) {
        ...
        String tweet = this.mc.next();
        for (int i = 1; i < numWords; i++) {
            if (!this.mc.hasNext()) {
                tweet = tweet + randomPunctuation();
                this.mc.reset();
            }
            tweet = tweet + " " + this.mc.next();
        }
        tweet = tweet + randomPunctuation();
        return tweet;
    }
    ...
    public void fixDistribution(List<String> tweet) {
         ...
         * probability distributions output a tweet in the order that we desire.
         * Our implementation does this by splitting us into 2 LNGs:
         * 1. TwitterBot LNG This LNG serves to make sure our punctuation is
         * output in the order we expect.
         * 2. MarkovChain LNG This LNG makes sure the tweets are output in the
         * proper order. This will be built by running `fixDistribution` on
         * the Markov Chain with punctuation replaced by null.
         */
