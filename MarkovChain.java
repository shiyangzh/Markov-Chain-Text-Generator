...
public class MarkovChain implements Iterator<String> {
    ...
    void addBigram(String first, String second) {
        ...
        if (chain.containsKey(first)) {
            ProbabilityDistribution<String> words = chain.get(first);
            words.record(second);
            chain.put(first, words);
        } else {
            ProbabilityDistribution<String> words = new ProbabilityDistribution<String>();
            words.record(second);
            chain.put(first, words);
        }
    ...
    public String next() {
        ...
        String temp = current;
        ProbabilityDistribution<String> words = chain.get(current);
        if (words != null) {
            current = words.pick(ng);
            return temp;
        } else {
            current = null;
            return null;
